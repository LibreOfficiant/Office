...
import Office 
from Office import Session as STATIC
from Office.UiDesktop import MsgBox

#~ from Office.devTools import mri, xray
	#~ ui(s.ProductName)  # n'affiche que les objects UNO
	#~ mri(s.ProductName)  # n'affiche pas les classes Python
	#~ xray(s.ProductName)  # n'affiche pas les classes Python

_L10N = True  # Show L(ocalisatio)n configuration information
_OFFICE = True  # Show (Libre/Open)Office configuration information
_PRODUCT = True # Show Product configuration information
_DIRS = True  # Show PathSettings information
_VARS = True  # Show PathSubstitution information

_LIBRE_OFFICE = 'LibreOffice'
_OPEN_OFFICE = 'OpenOffice'

def get_SessionProperties():
    ...
    if s.OfficeName == _LIBRE_OFFICE:
        ...
        msg += '\nOfficeRevisionSuffix: %s  aka product.SetupExtension' % s.OfficeRevisionSuffix
    ...
    """
    from Office.model.ooSession import _Configurations as Configs
    c = Configs()
    msg = ''
    ...
    from Office.model.ooSession import _Paths as Util
    u = Util()
    msg = ''
...
"""def call_SessionMethods():

	Office.Session.createUnoService('')
	MsgBox( "UiDesktop.MsgBox static method call" )
	msg = __file__
	msg += "\n%s" % STATIC.ConvertFromURL(__file__)
	msg += "\n%s" % s.SysExecutable
	msg += "\n%s" % STATIC.ConvertToURL(s.SysExecutable)
	MsgBox( msg, "This program name System executable are")"""
